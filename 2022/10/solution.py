...
def read_operations(operations, file_name):
...
def generate_signal(file_name):
    registers = dict({'x':1})
    yield registers['x'], ''
    for operation, params in read_operations(generate_operations(), file_name):
...
def run_part1(file_name):
    ...
    signal = list(generate_signal(file_name))
    print_signal(signal)
...
def run_part2(file_name):
    ...
    signal = list(map(lambda x: x[0],generate_signal(file_name)))
    # print(signal)
...
print("part 1")
# [1, 1, 4, 4, -1]
print_signal(generate_signal('small-example.txt'))
# 13140
